...
class RankingViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        for player in male_players:
            ...
            results = PlayerTournamentResult.objects.filter(
                player=player,
                tournament_date__gte=one_year_ago
            ).order_by('-tournament_date')[:6]  # Wybieramy 6 ostatnich turniejów
        ...
        sorted_male_rankings = sorted(male_rankings.items(), key=lambda x: x[1], reverse=True)
        ...
        # Sortowanie zawodników na podstawie punktów
        sorted_female_rankings = sorted(female_rankings.items(), key=lambda x: x[1], reverse=True)

        # Tworzenie ostatecznego słownika rankingowego
        final_female_rankings = {
            position + 1: {'user_id': player_id, 'points': points}
            for position, (player_id, points) in enumerate(sorted_female_rankings)
        }
    ...
    def get_last_ranking(self, request):
        ...
        last_ranking = Ranking.objects.filter(gender=gender).order_by('-date').first()

        if not last_ranking:
            return Response({"error": "No rankings found"}, status=status.HTTP_404_NOT_FOUND)
