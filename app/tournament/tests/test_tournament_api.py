...
class PrivateTournamentAPITest(TestCase):
    ...
    def test_updating_tournament_without_permission(self):
        ...
        user2 = create_user(
            email="anotheruser@example.com", password="Test123123"
        )
...
class TournamentTeamTestCase(TestCase):
    ...
    def test_create_team(self):
        ...
        payload = {"players": [self.player1.id, self.player2.id]}
        ...
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            self.tournament.teams.filter(
                players__in=[self.player1, self.player2]
            ).exists()
        )
...
class RemoveTeamFromTournamentTests(TestCase):
    ...
    def setUp(self):
        ...
        # Ustawiamy URL
        self.url = reverse(
            "tournament:tournament-remove-team",
    ...
    def test_remove_team_not_deletes_if_part_of_other_tournaments(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(
            self.team, other_tournament.teams.all()
        )  # Drużyna pozostaje w innym turnieju
...
class TournamentPointsTests(TestCase):
    def setUp(self):
        ...
        self.client = APIClient()
        self.organizer = create_user(
            email="organizer@example.com", password="password", user_type="OR"
        )
        self.tournament = create_tournament(
            name="Test Tournament", user=self.organizer
        )
        self.player1 = create_user(
            email="player1@example.com", password="123TestPass", user_type="PL"
        )
        self.player2 = create_user(
            email="player2@example.com", password="123TestPass", user_type="PL"
        )
        self.team = Team.objects.create()
        self.team.players.set([self.player1, self.player2])
        self.tournament.teams.add(self.team)
        self.url = reverse(
            "tournament:tournament-award-points",
            kwargs={"pk": self.tournament.id},
        )
    ...
    def test_award_points_success(self):
        ...
        payload = {"team_results": [{"team_id": self.team.id, "position": 1}]}

        res = self.client.post(self.url, payload, format="json")
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(PlayerTournamentResult.objects.count(), 2)
        player1_result = PlayerTournamentResult.objects.filter(
            player=self.player1
        ).first()
